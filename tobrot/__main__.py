...
if __name__ == "__main__" :
    ...
    incoming_purge_message_handler = MessageHandler(
        incoming_purge_message_f,
        filters=Filters.command(["purge@BurnaBoyBot"]) & Filters.chat(chats=AUTH_CHANNEL)
    )
    ...
    status_message_handler = MessageHandler(
        status_message_f,
        filters=Filters.command(["status@@BurnaBoyBot"]) & Filters.chat(chats=AUTH_CHANNEL)
    )
    ...
    rename_message_handler = MessageHandler(
        rename_message_f,
        filters=Filters.command(["rename@BurnaBoyBot"]) & Filters.chat(chats=AUTH_CHANNEL)
    )
    app.add_handler(rename_message_handler)
    #
    upload_document_handler = MessageHandler(
        upload_document_f,
        filters=Filters.command(["upload@BurnaBoyBot"]) & Filters.chat(chats=AUTH_CHANNEL)
    )
    app.add_handler(upload_document_handler)

    help_text_handler = MessageHandler(
        help_message_f,
        filters=Filters.command(["help@BurnaBoyBot"]) & Filters.chat(chats=AUTH_CHANNEL)
    )
    ...
    save_thumb_nail_handler = MessageHandler(
        save_thumb_nail,
        filters=Filters.command(["savethumbnail@BurnaBoyBot"]) & Filters.chat(chats=AUTH_CHANNEL)
    )
